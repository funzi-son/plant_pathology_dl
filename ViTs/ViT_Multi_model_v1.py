# -*- coding: utf-8 -*-
"""
Created on Wed Aug  2 13:35:50 2023
...
# def mlp_2(x, hidden_units, dropout_rate):
#     for units in hidden_units:
#         x = layers.Dense(units, activation=tf.nn.gelu)(x)
#         x = layers.Dropout(dropout_rate)(x)
#     return x
...
class Patches_2(layers.Layer):
    def __init__(self, patch_size):
        super(Patches_2, self).__init__()
        self.patch_size = patch_size
        ##############################################
    def get_config(self):
        config = super().get_config().copy()
...
class PatchEncoder_2(layers.Layer):
    def __init__(self, num_patches, projection_dim):
        super(PatchEncoder_2, self).__init__()
        self.num_patches = num_patches
        self.projection = layers.Dense(units=projection_dim)
        self.position_embedding = layers.Embedding(
            input_dim=num_patches, output_dim=projection_dim
...
def create_vit_classifier_p(p_type, fig_size,inputs):
    ...
    # Add MLP.
    features_1 = mlp(representation, hidden_units=mlp_head_units, dropout_rate=0.5)
...
def create_vit_classifier_d(d_type, fig_size,inputs):
    # input_shape = (fig_size, fig_size, 3)
    ...
    # Create patches.
    # patches = Patches(patch_size)(augmented)
    patches_2 = Patches_2(patch_size)(inputs)
    # Encode patches.
    encoded_patches = PatchEncoder_2(num_patches, projection_dim)(patches_2)
    ...
    representation = layers.Dropout(0.5)(representation)
    # Add MLP.
    features_d = mlp(representation, hidden_units=mlp_head_units, dropout_rate=0.5)
...
for model_name in model_list:
    if item == "PlantDoc_original":
        trainX, trainDiseaseY, trainPlantY, fig_size, model_dir,  dir_save, p_type, d_type, testX, disease_y, plant_y = load_data.load_data(model_name, server,item,obj,tip,fig_size = 256)
        ...
        data, diseaseLabels, PlantLabels, fig_size, model_dir, categ, dir_save, p_type, d_type  = load_data.load_data(model_name, server,item,obj,tip,fig_size = 256)
    ...
    es = callbacks.EarlyStopping(monitor='val_loss', min_delta=0, patience=30, verbose=1, mode='min') #,restore_best_weights=True
    ...
    for g in range(0,times):
        ...
        # def build_Plant_branch(p_type, fig_size):
        features = create_vit_classifier_p(p_type, fig_size,inputs)
        PlantBranch = Dense(len(p_type), "softmax", name="plant_output")(features)

        features = create_vit_classifier_d(d_type, fig_size,inputs)
        DiseaseBranch = Dense(len(d_type), "softmax", name="disease_output")(features)


        model = Model(inputs=inputs,
            outputs=[PlantBranch, DiseaseBranch],
            # name="PlantNet"
            )
        ...
        H = model.fit(x=trainX,
                      y={"plant_output": trainPlantY, "disease_output": trainDiseaseY },batch_size=bat_si,
